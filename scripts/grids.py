...
class StochOccupancyGrid2D(object):
    def __init__(self, resolution, width, height, origin_x, origin_y,
                window_size, probs, thresh=0.5, speed=1.0):
        ...
        self.thresh = thresh
        self.speeds = np.full((height, width), speed)

    def snap_to_grid(self, x):
        return (self.resolution*round(x[0]/self.resolution),
                self.resolution*round(x[1]/self.resolution))
    ...
    def is_free(self, state):
        ...
        for dx in range(lower,upper+1):
            for dy in range(lower,upper+1):
                x, y = self.snap_to_grid([state[0] + dx * self.resolution,
                                          state[1] + dy * self.resolution])
                grid_x = int((x - self.origin_x) / self.resolution)
                grid_y = int((y - self.origin_y) / self.resolution)
                if grid_y>0 and grid_x>0 and grid_x<self.width and grid_y<self.height:
                    p_total *= \
                      (1.0-max(0.0,float(self.probs[grid_y * self.width + grid_x])/100.0))
        return (1.0-p_total) < self.thresh

    def set_speed(x, y, speed):
        x_min = max(0, x-10)
        x_max = min(self.width, x+11)
        y_min = max(0, y-10)
        y_max = min(self.height, y+11)
        self.speeds[x_min:x_max, y_min:y_max].fill(speed)
